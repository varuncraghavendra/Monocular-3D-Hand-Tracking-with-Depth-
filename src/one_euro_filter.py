"""1 Euro Filter (EXT 2) — adaptive low-pass smoother for noisy keypoints.

Reference: Casiez et al., CHI 2012. Cutoff rises with velocity, so fast motions
stay responsive while slow motions are heavily smoothed.
"""
...
class LowPassFilter:
    ...
    def apply(self, x: np.ndarray, alpha: np.ndarray) -> np.ndarray:
        # Single-pole IIR: y[n] = a*x[n] + (1-a)*y[n-1], broadcastable over any shape.
...
class OneEuroFilter:
    ...
    def _alpha(self, cutoff) -> np.ndarray:
        # Convert cutoff frequency (Hz) to IIR alpha coefficient at current rate.
    ...
    def __call__(self, x: np.ndarray, freq: float | None = None) -> np.ndarray:
        # Adaptive cutoff: f_c = min_cutoff + beta * |velocity|. Call with
        # freq=current_fps to update the sampling rate on the fly.
